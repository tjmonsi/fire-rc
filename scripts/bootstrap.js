...
(function () {
  ...
  function logRejectedPromises() {
    unhandledRejections.forEach(function (reason) {
      FIRERC.Analytics.trackError('UnhandledPromiseRejection', reason);
    });
  ...
  function lazyLoadWCPolyfillsIfNecessary(callback) {
    ...
    if (!(FIRERC.Util.getChromeVersion() &&
      FIRERC.Util.getChromeVersion() >= 46 || FIRERC.Util.getFirefoxVersion() && FIRERC.Util.getFirefoxVersion() >= 40)) {
      var parent = document.querySelector('body');
  ...
  window.addEventListener('offline', function () {
    if (FIRERC.Elements && FIRERC.Elements.Template.$.toast) {
      FIRERC.Elements.Template.$.toast.show(
  ...
  window.addEventListener('beforeinstallprompt', function (event) {
    FIRERC.Analytics.trackEvent('installprompt', 'fired');

    event.userChoice.then(function (choiceResult) {
      // choiceResult.outcome will be 'accepted' or 'dismissed'.
      // choiceResult.platform will be 'web' or 'android' if the prompt was
      // accepted, or '' if the prompt was dismissed.
      FIRERC.Analytics.trackEvent('installprompt', choiceResult.outcome,
